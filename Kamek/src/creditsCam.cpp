...
dCreditsCamera_c *dCreditsCamera_c::instance = 0;

dCreditsCamera_c *dCreditsCamera_c::build() {

	void *buffer = AllocFromGameHeap1(sizeof(dCreditsCamera_c));
	dCreditsCamera_c *c = new(buffer) dCreditsCamera_c;
...
#include <profile.h>
Profile CreditsCameraProfile = Profile((buildFunc)&dCreditsCamera_c::build, ProfileId::STAFFCREDITS_CAMERA, 0, ProfileId::STAFFCREDITS_CAMERA, ProfileId::STAFFCREDITS_CAMERA, 0, "STAFFCREDITS_CAMERA", 0);



dCreditsCamera_c::dCreditsCamera_c() {
...
int dCreditsCamera_c::onDelete() {
	return true;
}


int dCreditsCamera_c::onExecute() {
	OSReport("currentX: %f, currentY: %f, currentZ: %f\n", currentX, currentY, currentZ);
	OSReport("zoomLevel: %f, near: %f, far: %f\n", zoomLevel, projection2d.near, projection2d.far);


	if(isBound) {
		currentX = posStart.x + (boundTo->x - boundStart.x);
		currentY = posStart.y + (boundTo->y - boundStart.y);
		currentZ = posStart.z + (boundTo->z - boundStart.z);

		if(currentX < bdMin.x) currentX = bdMin.x;
		if(currentY < bdMin.y) currentY = bdMin.y;
		if(currentZ < bdMin.z) currentZ = bdMin.z;

		if(currentX > bdMax.x) currentX = bdMax.x;
		if(currentY > bdMax.y) currentY = bdMax.y;
		if(currentZ > bdMax.z) currentZ = bdMax.z;
	}
...
void dCreditsCamera_c::panToBounds(float left, float top, float right, float bottom) {
	// Pad it a bit
	left -= 64.0f;
	right += 64.0f;
	top -= 48.0f;
...
void dCreditsCamera_c::panToPosition(float x, float y, float zoom) {
	panFromX = currentX;
	panFromY = currentY;
...
void dCreditsCamera_c::calculateScreenGeometry() {
...
void dCreditsCamera_c::doStuff(float something) {
	camTarget = (Vec){
...
void dCreditsCamera_c::generateCameraMatrices() {
...
void dCreditsCamera_c::updateCameras() {
	...
	nw4r::g3d::Camera cam1(GetCameraByID(1));
	camera3d.assignToNW4RCamera(cam1);
	screen.loadIntoCamera(cam1);
}
