...
bool LevelSpecial_Execute(LevelSpecial *self) {
	if (self->keepTime > 0) 
		TimeKeeper::instance->paused = true;
	else
		TimeKeeper::instance->paused = false;
...
void LevelSpecial_Update(LevelSpecial *self) {
	...
	u8 newEvState = 0;
	if (self->eventFlag == 0 || dFlagMgr_c::instance->flags & self->eventFlag)
		newEvState = 1;
	...
	if (newEvState == ACTIVATE)
	{
		...
		switch (self->type) {
			...
			case 4:											// Set Time
				TimeKeeper::instance->setTime((self->setTime << 0xC) - 1); // Possibly - 0xFFF?
				break;
