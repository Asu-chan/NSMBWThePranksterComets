/*---------------------------------------------------------------------------*
  ...
  $Log: enc.h,v $
  Revision 1.9  2008/02/27 13:46:45  wada_jumpei
  Since other library including NWC24 cannot hundle ENC_ERR_NOT_LOADED, obsoleted it.
  But in order to avoid being overwritten by other error, it is still defined.

  Revision 1.8  2008/02/12 13:14:50  wada_jumpei
  Supported Korean and Chinese.
   Korean: UHC (CP949)
   Chinese: EUC-CN + additional Kanji
...
ENCResult ENCConvertStringGb2312ToUnicode(u16* dst, s32* dstlen, const u8* src, s32* srclen);
ENCResult ENCConvertStringUnicodeToGb2312(u8* dst, s32* dstlen, const u16* src, s32* srclen);

/* Korean */

ENCResult ENCConvertStringUhcToUnicode(u16* dst, s32* dstlen, const u8* src, s32* srclen);
ENCResult ENCConvertStringUnicodeToUhc(u8* dst, s32* dstlen, const u16* src, s32* srclen);
...
#define ENC_STRIP_TABLE_CN \
    BOOL enc_tbl_cn_loaded = FALSE; \
    const u8* enc_tbl_cn_wctomb = NULL;   \
    const u8* enc_tbl_cn_mbtowc = NULL;   \
    const u16* enc_offset_cn = NULL; \
    const u16* enc_tbl_cnex_mbtowc = NULL;


#define ENC_STRIP_TABLE_KR_KANJI \
    BOOL enc_tbl_kr_loaded = FALSE; \
    const u8* enc_tbl_kr_wctomb2 = NULL;   \
    const u8* enc_tbl_kr_mbtowc2 = NULL;   \
    const u16* enc_offset_kr2 = NULL;

#define ENC_STRIP_TABLE_KR_UHC \
    BOOL enc_tbl_uhc_loaded = FALSE; \
    const u16* enc_tbl_uhc_mbtowc = NULL; \
    const u16* enc_offset_uhc = NULL;

#define ENC_STRIP_TABLE_KR \
    const u8* enc_tbl_kr_wctomb1 = NULL;   \
    const u8* enc_tbl_kr_mbtowc1 = NULL;   \
    const u16* enc_offset_kr1 = NULL; \
    ENC_STRIP_TABLE_KR_KANJI \
    ENC_STRIP_TABLE_KR_UHC
